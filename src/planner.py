...
class planner():
  def __init__ (self,sim):
    self.map=self.init_map(sim)
    return None

  def getPlan(self,list_rob_targets,sim):

    self.robotIDs=[a[0] for a in list_rob_targets]
    self.robotTargets=[list_rob_targets[i][1:] for i in range(len(list_rob_targets))]
    ...
    for robID,i in enumerate(self.robotIDs):
      ...
      for target in self.robotTargets[i-1]:
        
        # print "current_pos: ",current_pos
        current_pos=current_pos[:2]
        coordinates=target[:2]
        print "target: ",target
        ...
        plan_coordinates,path_length= self.astar2d(current_pos,coordinates,self.map)[:2]
  ...
  def coordinatesToString(self,plan):
    ...
    for next_pos in plan[1:]:
      if(next_pos[1]<curr_pos[1] and next_pos[0]==curr_pos[0]):
        plan_string.append('N')
      elif(next_pos[1]>curr_pos[1] and next_pos[0]==curr_pos[0]):
        plan_string.append('S')
      elif(next_pos[1]==curr_pos[1] and next_pos[0]>curr_pos[0]):
        plan_string.append('E')
      else:
        plan_string.append('W')
      curr_pos=next_pos
    return plan_string
